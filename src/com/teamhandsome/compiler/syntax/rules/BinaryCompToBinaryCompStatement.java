...
public class BinaryCompToBinaryCompStatement extends SyntaxRule{

	private static final int SIZE = 2;

	public BinaryCompToBinaryCompStatement() {
	...
	public boolean isRule(List<SyntaxNode> nodes, SyntaxNode next) {
		return nodes.size() >= 2 
				&& nodes.get(nodes.size() - 2).getType() == NodeType.BINARY_COMPARATOR 
				&& nodes.get(nodes.size() - 1).getType() == NodeType.SEMICOLON;
	}

	@Override
	public SyntaxNode applyRule(List<SyntaxNode> nodes) {
		return constructNode(NodeType.BINARY_COMP_STATEMENT, SIZE, nodes);
	}
